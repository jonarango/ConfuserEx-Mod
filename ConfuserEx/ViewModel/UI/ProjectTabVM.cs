...
namespace ConfuserEx.ViewModel {
	public class ProjectTabVM : TabViewModel {
		...
		public ICommand Remove {
			get {
				return new RelayCommand(() => {
					...
					if (MessageBox.Show(msg, "[ж]-Confuser", MessageBoxButton.YesNo, MessageBoxImage.Question) == MessageBoxResult.Yes) {
		...
		void AddModule(string file) {
			if (!File.Exists(file)) {
				MessageBox.Show(string.Format("File '{0}' does not exists!", file), "[ж]-Confuser", MessageBoxButton.OK, MessageBoxImage.Error);
				return;
			}
