...
namespace Confuser.CLI {
	internal class Program {
		...
		static int RunProject(ConfuserParameters parameters) {
			...
			Console.Title = "[ж]-Confuser - Running...";
		...
		class ConsoleLogger : ILogger {
			...
			public void Info(string msg) {
				WriteLineWithColor(ConsoleColor.Green, " [ INFO ] " + msg);
			}

			public void InfoFormat(string format, params object[] args) {
				WriteLineWithColor(ConsoleColor.Green, " [ INFO ] " + string.Format(format, args));
			}
			...
			public void Finish(bool successful) {
				...
				if (successful) {
					Console.Title = "[ж]-Confuser - Success";
					WriteLineWithColor(ConsoleColor.Green, "Finished " + timeString);
					ReturnValue = 0;
				}
				else {
					Console.Title = "[ж]-Confuser - Fail";
