        ...
        private class JunkDumpPhase : ProtectionPhase
        {
            ...
            protected override void Execute(ConfuserContext context, ProtectionParameters parameters)
            {
                foreach (ModuleDef moduleDef in parameters.Targets.OfType<ModuleDef>())
                {
                    ...
                    {
                        moduleDefMD.Name = "[ xnoobx.cc ]";
                        moduleDefMD.Assembly.Name = "[ xnoobx.cc ]";
                        ...
                        for (int i = 0; i <= num; i++)
                        {
                            for (int j = 0; j <= 50; j++)
                            {
                                ...
                                new TypeDefUser(Random(15) + "[ xnoobx.cc ]", Random(rnd.Next(1, 20)) + "<Module>", moduleDefMD.CorLibTypes.Object.TypeDefOrRef).Attributes = TypeAttributes.Public;
                                TypeDef item = new TypeDefUser(Random(rnd.Next(1, 25)) + "<Module>", Random(rnd.Next(1, 15)) + "[ xnoobx.cc ]", moduleDefMD.CorLibTypes.Object.TypeDefOrRef)
                                {
                                    Attributes = TypeAttributes.Public
                                };
                                TypeDef item2 = new TypeDefUser(Random(rnd.Next(1, 20)) + "[ xnoobx.cc ]", Random(rnd.Next(1, 15)) + "<Module>", moduleDefMD.CorLibTypes.Object.TypeDefOrRef)
                                {
                                    Attributes = TypeAttributes.Public
                                };
                            ...
                            moduleDefMD.EntryPoint.Name = Random(rnd1.Next(1, 10)) + "[ xnoobx.cc ]" + Random(300) + "[ xnoobx.cc ]" + Random(rnd1.Next(1, 10));
                        }
                    }
                    else
                    {
                        context.Logger.Warn("Junk Protection cannot be used on a dll");
                    }
                }
            }
